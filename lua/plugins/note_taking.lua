...
local config = {
  base_paths = {
    Windows = {
      home = os.getenv("USERPROFILE") or "C:\\Users\\dylan",
      notes = "D:\\notes",
      projects = "C:\\Users\\dylan\\Documents\\Projet\\Work\\Projet",
      config = os.getenv("APPDATA") or "C:\\Users\\dylan\\AppData\\Roaming",
    },
    WSL = {
      home = os.getenv("HOME"),
      notes = "/mnt/d/notes",
      projects = "/mnt/c/Users/dylan/Documents/Projet/Work/Projet",
      config = os.getenv("HOME") .. "/.config",
    },
    Linux = {
      home = os.getenv("HOME"),
      notes = os.getenv("HOME") .. "/Documents/Notes",
      projects = os.getenv("HOME") .. "/documents/Projet/Work/Projet",
      config = os.getenv("HOME") .. "/.config",
    },
  },
}
...
local function build_path(base, ...)
  ...
  local os_base = config.base_paths[os_name][base]
  local segments = { ... }

  if os_name == "Windows" then
    return os_base .. "\\" .. table.concat(segments, "\\")
  else
    return os_base .. "/" .. table.concat(segments, "/")
  end
end
...
local function create_path_config()
  ...
  local function get_paths(path_table)
    return os_utils.get_setting(path_table)
  end
  ...
  -- },
  {
    ...
    opts = {
      ...
      attachments = {
        ...
        img_folder = paths.obsidian_attachments_path, -- This is the default
